...
class Palletizer(Generic, EasyResource):
    ...
    MODEL: ClassVar[Model] = Model(
        ModelFamily("shrews-testing", "palletizer-102"), "palletizer"
    )
    ...
    def new(
        ...
        # The cell's sensors. You will use these later in the course.
        self.box_detect = None
        self.pallet_empty = None
        self.tray_dock = None
        return self
    ...
    async def do_command(
        ...
        **kwargs
        ...
        # place: put one box in the next slot, answer the slot.
        if verb == "pack":
            return {"packed": await self.pack()}

        # Anything unrouted gets an answer, never a stack trace.
    ...
    async def pick(self, seq=0):
        ...
        # The infeed box leaves the moment the cups take it, and the
        # next one starts down the conveyor during the carry.
    ...
    # sensor being absent, which is why this same file runs before and
    # after the sensors page.
